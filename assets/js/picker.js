...
window.onload = function() {
	...
	$header.setAttribute('class', `texture-yuhui`);
...
xhr.open('get', '/feed.xml', true); //M这样相当于将所有posts下的post聚集在feed.xml文件,变相获取site.posts等jekyll变量(所有内容都可获取)
xhr.send();//M告诉对方,需要返回给我feed.xml里的数据.上面的xhr.responseXML即是返回的feed.xml里的数据
...
function searchMatching(arr1, arr2, input) {
    ...
    for (i = 0; i < itemLength; i++) {
        ...
        if (arr1[i].search(input) !== -1 || arr2[i].search(input) !== -1) {
            ...
            // if (arr1[i].search(input) !== -1) {
            	// console.log(i)
            if (arr2[i].search(input) !== -1) {
                var arr = arr2;
                a = 1;
                // console.log(i)
                // console.log(arr1[i].search(input) !== -1)
            } 
            if(arr1[i].search(input) !== -1){
            	// console.log(1)
            	if(a == 1){//说明此时同时满足内容和标题匹配
            		// console.log(i)
            		arr_double[i] = 1;
            	} else{
            		var arr = arr1;
    ...
    for (i = 0; i < arrResults.length; i++) {
        var itemDiv = tmpDiv.cloneNode(true);
        if(arr_double[indexItem[i]] == 1){// M有both说明同时匹配到标题和内容
	        itemDiv.innerHTML = '<b>《' + arrTitles[indexItem[i]] +
	            '》both</b><br />' + arrResults[i];
        } else{
	        itemDiv.innerHTML = '<b>《' + arrTitles[indexItem[i]] +
	            '》</b><br />' + arrResults[i];
        }
